(function () {
	...
	function StartPageCtrl($scope, $mdMedia, $timeout, web3Service, market) {
		...
		function updateWeb3Status() {
			...
			let isUsingMetaMask = web3Service.getProviderName() == "MetaMask";
			let isFirefox = typeof InstallTrigger !== 'undefined';
			let isEdge = window.navigator.userAgent.indexOf('Edg/') > -1;
			let isChrome = !isEdge && !!window.chrome && (!!window.chrome.webstore || !!window.chrome.runtime) && !navigator.brave;
			let isBrave = !isEdge && !!window.chrome && (!!window.chrome.webstore || !!window.chrome.runtime) && !!navigator.brave;
		...
		// Check if the given how to section is expanded
		function checkHowToSectionExpanded(section) {
			for(let p in _this.hideHowTo[section]) {
				if(!_this.hideHowTo[section][p]) return true;
			}
			return false;
		}
		
		// Toggles the expand/collapse state of the given how to section
		function toggleHowToSection(section) {
			let hide = checkHowToSectionExpanded(section);
			for(let p in _this.hideHowTo[section]) {
				_this.hideHowTo[section][p] = hide;
			}
		}
